...
As effect size ($\beta_1$) increases, the probability we reject the null approaches 1, and therefore the power increases significantly. By the time $\beta_1 = 6$ given the experiment conditions, we are almost certain to reject the null. 
...
The average $\hat{\beta_1}$ tends to be larger among $\hat{\beta_1}$ which rejected the mean. This is especially true among $\beta_1 = 1,2,3$. For these $\beta_1$'s, the null will only be rejected if $\hat{\beta_1}$ is much larger than 0, and therefore larger than $\beta_1$. For larger values of $\beta_1$ the null is almost always rejected, as this criteria will already be fufilled. Therefore, as $\beta_1$ increases, the average of all $\hat{\beta_1}$s approaches that of the $\hat{\beta_1}$ which rejected the mean. At $\beta_1 = 0$, we are equally likely to reject because of very large or very small values, and this results in the two averages being essentially equal. 
